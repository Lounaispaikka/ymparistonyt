...
class Ymparisto_arviointi_kysely extends \Lougis\DB_DataObject_Wrapper 
{
    ...
    /*
     * There can be many rounds of queries. We need to get the id of the most recent
     * to be able to show the results of the round correctly.
     *
     * Note!! The round ids are give out sequentially. Thus this method assumes the most recent
     * round is equalent to highest id.
     *
     * @return id of the most recent round(kierros)
     */
    public function getMostRecent($PageId){
        $Kysely = new \Ymparisto_arviointi_kysely();
        $Kysely->selectAdd("kierros_id");
        $Kysely->whereAdd("page_id = ".$PageId);
        $Kysely->orderBy('kierros_id DESC');
        $Kysely->limit(1);
        $Kysely->find();
        $Kysely->fetch();
        return $Kysely->kierros_id;
    }   
}
